  ...
  return (
    <div className="container" style={{ paddingTop: 40, paddingBottom: 60 }}>
      ...
      <p className="muted" style={{ marginBottom: 28 }}>
        Personal space — only you can see this, unless you mark an article as public.
      </p>

      <div className="card" style={{ marginBottom: 28 }}>
        <h3 style={{ fontSize: 14, fontWeight: 600, marginBottom: 14 }}>Add manually</h3>
        <form onSubmit={addArticle}>
          <div className="grid" style={{ gridTemplateColumns: '2fr 2fr 1fr auto', gap: 10, alignItems: 'end' }}>
            <div>
              <label className="label">Title</label>
              <input className="input" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder="Article title" />
            </div>
            <div>
            ...
            <div>
              ...
              <input className="input" value={form.tag} onChange={(e) => setForm({ ...form, tag: e.target.value })} placeholder="e.g. Heart attack" />
            </div>
            <button className="btn btn-primary" type="submit">
              <i className="ti ti-plus" style={{ fontSize: 14 }} aria-hidden="true"></i> Save
            </button>
          </div>
        </form>
      </div>
      ...
      {loading ? (
        ...
        <div className="empty-state">
          ...
          No saved articles yet. Find something in Search and click the save icon.
        </div>
      ) : (
        <div className="grid grid-2">
          {filtered.map((a) => (
            <div key={a.id} className="card" style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
              ...
              <button className="btn btn-sm btn-ghost btn-danger" onClick={() => deleteArticle(a.id)} aria-label="Delete">
