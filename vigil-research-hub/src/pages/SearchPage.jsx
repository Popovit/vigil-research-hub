...
export default function SearchPage() {
  ...
  async function quickSave(source) {
    if (!user) {
      setSavedMsg({ type: 'error', text: 'Влез в акаунта си, за да запазваш статии.' })
    ...
    const { error } = await supabase.from('saved_articles').insert({
      user_id: user.id,
      title: `${query} — търсене в ${source.name}`,
    ...
    if (error) {
      setSavedMsg({ type: 'error', text: 'Грешка при запазване.' })
    } else {
      setSavedMsg({ type: 'success', text: `Запазено в "${query}"` })
    }
  ...
  return (
    <div className="container" style={{ paddingTop: 40, paddingBottom: 60 }}>
      ...
      <p className="muted" style={{ marginBottom: 28 }}>
        Едно търсене, дванадесет бази данни. Научна, регулаторна и пазарна информация за predictive healthcare AI.
      </p>

      <input
        className="input"
        style={{ fontSize: 16, padding: '14px 16px', marginBottom: 14 }}
        placeholder="напр. mmWave heart rate prediction"
        value={query}
      ...
      <div className="flex gap-8" style={{ flexWrap: 'wrap', marginBottom: 28, paddingTop: 14, borderTop: '1px solid var(--border)' }}>
        <span className="muted" style={{ fontSize: 12, marginRight: 4 }}>Филтър:</span>
        {cats.map((c) => (
          <button
            key={c}
        ...
        {visibleSources.map((s) => (
          <div key={s.id} className="card">
            ...
            <div className="flex gap-8">
              ...
              >
                Отвори <i className="ti ti-external-link" style={{ fontSize: 13 }} aria-hidden="true"></i>
              </a>
              <button
